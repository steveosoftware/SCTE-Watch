...
- **A media playlist handed over directly is polled like any other live target** (fixed 2026-09-10). When the URL has no `#EXT-X-STREAM-INF` it isn't a master, and that branch used to pass a hardcoded `() => false` liveness predicate — so a live playlist loaded directly was fetched once and then frozen on that snapshot, leaving every across-polls check (sequence continuity, staleness, cue dedupe) with nothing to compare. It now uses the same `isLiveHlsPlaylist` (`#EXT-X-ENDLIST`) test as a variant reached through a master.
