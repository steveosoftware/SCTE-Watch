...
- **Download CSV** button — one row per programme slot, both sides side by side with a verdict column (`ok` / `time-drift` / `tms-mismatch` / `no-tms-mapping` / `only-in-xumo` / `only-in-gracenote`). The original shell workflow's whole deliverable was a CSV report, so the findings stay exportable rather than trapped in a `<pre>`. `buildComparisonCsv()` in `epg.js` (pure, unit-tested including comma/quote escaping).
- Long lists cap at 50 entries **and say so** (`… and N more`) rather than silently truncating, which would understate real drift. The CSV always carries the full set.
