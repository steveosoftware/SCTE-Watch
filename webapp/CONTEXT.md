...
Latest push to `roadmap` (2026-09-10): the EPG panel now anchors on `#EXT-X-PROGRAM-DATE-TIME` when the stream publishes one, which also fixed a false `wrong-asset` alarm at every programme boundary — see the EPG panel section and ROADMAP.md's dated entry.
...
This is deployed publicly, so **the app holds no third-party API credentials at all**. Any integration needing a key (Gracenote today, anything similar later) takes the *user's own* key via a UI input — `type="password"`, never a Lambda env var, never sent to our own backend, never committed. Per-user keys mean no shared quota to abuse, no secret in our infrastructure, and nothing for us to rotate. Decided 2026-08-20; it supersedes the earlier "Lambda holds the key" plan that ROADMAP.md used to describe. The operator gates the deployed site behind Amplify's own password protection separately (they manage that themselves — not something this codebase configures).

**Amended 2026-08-28: the key IS now persisted in the user's own browser** (`localStorage`, keys `scte-watch.gracenote-api-key` / `scte-watch.gracenote-remember-key`), opt-out via a visible "Remember in this browser" checkbox that defaults on; unticking wipes the stored copy immediately and the opt-out itself is remembered. Requested by the operator — retyping a long opaque key every session was the main friction in using the panel. This relaxes the earlier "never localStorage" rule and nothing else: the key still goes only to Gracenote, never to our backend or repo. What it costs is that the key now outlives the tab, so on a shared machine, or against any XSS on this origin, the exposure window becomes every future visit rather than one session — which is why the toggle is visible rather than implicit. Only the key is persisted; no other field is.
...
```bash
npm test            # unit tests (fast, no browser, no network) — 304 passing
npm run test:e2e     # e2e — needs `npx playwright install chromium` once first — 23 passing
```
...
webapp/
  ...
  public/
    index.html            page shell, five panels + glossary/explainer modals + the explainer content templates (EPG above VAST — see the panel section)
...
Order matters and is deliberate: the EPG panel sits **above** the VAST/VMAP one (moved 2026-08-28, at the operator's request) — schedule/drift work is the day-to-day job, ad-response validation the occasional one.

### 1. HLS & DASH Stream Tester
- URL input (auto-detects HLS vs DASH by extension, or force either), Load & Play / Stop.
- The URL input **takes the room the row has spare** (`flex: 1`, widened 2026-08-28 at the operator's request) — stream URLs are long and differ only in their tail (channel id, variant, token), so reading back what's actually in the field is the point of it. Same rule now covers the EPG panel's URL and key inputs. The rule it replaced was dead: it keyed on `#master-url`/`#interval`, ids that no longer exist anywhere in the page.
...
### 4. EPG drift — what's playing vs what's scheduled (shipped 2026-08-26, reworked 2026-08-27)
...
- **It polls because it can't assume a wallclock anchor.** Plenty of streams publish `#EXT-X-PROGRAM-DATE-TIME` (playlist-level or per-segment) and plenty don't — the Xumo linear channels this was built against are bare: `VERSION`, `TARGETDURATION`, `MEDIA-SEQUENCE`, `EXTINF`, segment URLs, nothing else (verified). The instant that's always observable is "now" at the live edge. A single check therefore yields a yes/no; watching across polls catches the asset transition and converts it into a **drift figure in seconds**.
- **The panel reads `#EXT-X-PROGRAM-DATE-TIME` when the stream publishes one** (2026-09-10), falling back to live-edge interpolation when it doesn't. `parseMediaPlaylistAssets()` returns a `timingSource` of `"pdt"` or `"live-edge"` saying which, and the panel announces it once per run. One tag anchors the whole window — PDT formally timestamps only the segment after it, and the rest accumulate `#EXTINF` from there (same shape as `scte35.js`'s `findCueWallclocks()`). A `#EXT-X-DISCONTINUITY` invalidates the anchor since durations don't compose across one; a PDT after it re-anchors, and without one the **whole** playlist falls back rather than returning a part-null timeline.
- **Transitions are identified by `#EXT-X-MEDIA-SEQUENCE`, not by time.** A transition stays visible for as long as it sits inside the playlist window (~60s), and its wall-clock time is an *estimate* that shifts by up to a segment duration between polls as the window slides. Keying on the media-sequence number of the new asset's first segment gives a stable identity, so an event is reported exactly once and the drift figure stops jittering; the first sighting is kept as the best estimate, since it interpolates over the fewest segments. (Bug found in live testing 2026-08-28: the transition log sat outside the "only on change" guard and re-reported the same event on every poll, at three different times.)
- **Without a PDT the timeline is interpolated backwards from the live edge**: the last segment's end is anchored at `now`, then `#EXTINF` durations are accumulated backwards. Two distinct error sources, worth keeping apart because only one is fixable. **Resolution** is ±1 segment — the finest grain a playlist describes — and no anchor escapes it. **Anchor bias** is the fallback assuming the live edge is `now` when the packager is really tens of seconds behind; it shifts *every* timestamp late by the same amount, and PDT removes it entirely. Earlier notes here described the accuracy as "roughly a segment duration", which only accounted for the first and understated the second.
- **A programme boundary inside the packager latency used to false-alarm** (fixed 2026-09-10). The asset id is read at the live edge — content encoded seconds ago — while `comparePlaybackToSchedule()` asked the schedule about `now`. Different instants. For the length of the packager latency after a rollover the live edge still legitimately carries the outgoing programme while the schedule has moved on, so a perfectly correct channel reported `wrong-asset` (the red verdict) once per boundary. The lookup now uses the live edge's own media time (`playback.liveEdgeMs`, exposed for exactly this). Under live-edge anchoring `liveEdgeMs === nowMs`, so no-PDT streams are bit-for-bit unchanged — including this false positive, which needs an anchor those streams don't publish. Regression-pinned in both suites.
- **Verdicts**: `match`, `wrong-asset` (something else *that is scheduled* is playing), `unscheduled` (playing something the schedule doesn't list at all), `no-asset-id`, `no-schedule`.
- **`unscheduled` is usually NOT an alarm** — learned from the first live run (2026-08-28). An EPG schedules *programmes*, not break filler, so a channel sitting in an ad break plays an asset that legitimately appears nowhere in the schedule. Real drift much more often surfaces as `wrong-asset`. The panel resolves the playing asset's title through Xumo's public asset endpoint (`valencia-app-mds.xumo.com/v2/assets/asset/{id}.json`, one lookup per asset *change*, cached, best-effort) and uses `contentType` to tell the cases apart: `SHORT-FORM` → filler, shown neutrally; `EPISODIC` → a real finding, shown as a warning. Concrete example: channel 88893069 was playing `XM05M7E0PC09SI` = *"Forensic Files EN Ad Slate 2026 2 Minutes"*, SHORT-FORM/120s, while Gracenote expected `XM0INFHT3SEGWZ` = *"Palm Saturday"*, EPISODIC/1275s — an ad break, not drift.
- The title lookup also makes the output legible: opaque `XM…` ids render as real names.
- **Both sides get titles.** `/Schedules` returns TMS ids but *no* titles — those come from a second `/Programs?tmsId=A,B,C` call, batched over the distinct ids (`PROGRAMS_BATCH_SIZE` = 50, chunked so a 14-day window can't blow the URL length) and issued once per schedule load. Entirely best-effort: a failure costs titles, never the comparison. Forensic Files returns no `<episodeTitle>`, only `<title type="full">` + `<episodeInfo season number>`, so `displayTitle` falls back to `Forensic Files S13E41`. Note Gracenote's `<titles>` block also carries truncated `type="red"` variants (down to `"Forensic"`) — the parser explicitly prefers `type="full"`.
- Nice side effect: the two sources are complementary — Xumo's asset lookup gives the *episode name* ("Shoe-In For Murder"), Gracenote gives the *season/episode number* (S13E41). The panel shows both.
- **Verified end to end against production** (2026-08-28): playback channel `88893069` + Gracenote `prgSvcId 157905` → `✓ correct asset playing · XM0YNWLNHUP8YD — Shoe-In For Murder · expected XM0YNWLNHUP8YD (Forensic Files S13E41)`, 52 of 52 titles resolved, no console errors, key absent from the DOM. **That pairing is confirmed correct** — an earlier `UNSCHEDULED` on the same pair was the channel sitting in an ad break, not a mispairing. XMLTV mode verified the same way, including correctly flagging a deliberately mispaired channel.
- **Channel pairing can't be verified in Gracenote mode.** Nothing links the Xumo channel id (in the playback URL) to Gracenote's `prgSvcId`. `/Sources` does return a **`callSign`** (e.g. `XRGFF` for 157905) which is the same callsign that appears in the XMLTV URL pattern `{xumoChannelId}_{callSign}.xml` — a partial bridge, but it still doesn't yield the channel id. In XMLTV mode the display-name guardrail works; in Gracenote mode the panel says it can't check rather than reporting confident nonsense.
...
- **Verdicts are colour-coded** (added 2026-08-28): green for `match`, red for a real finding (`wrong-asset`, or `unscheduled` that isn't filler), neutral for break filler and for the states where the tool can't tell (`no-asset-id`/`no-schedule`). Banner and log line share one `toneFor()` rule so they can't disagree about severity. Amber (`.status.warn`) is left meaning what it always did — *the tool* had trouble — which is a different thing from the channel being wrong. Colour is never the only signal: every state is also named in words, since red/green is precisely the pair a colour-blind viewer can't separate. Log tints are inline spans, not full-width bands, so the `<pre>` keeps real newlines in its text content — the transition-dedupe test reads that element line by line.
- Logs only on **change**, not every poll — a monitor that reprints an unchanged line every few seconds buries the events that matter. **Download drift CSV** exports the observation log (`buildObservationCsv()`).
- **"How it works"** (added 2026-08-28) — a button in the panel heading opens a modal explaining the mechanics: what it compares, where the asset id comes from, why it polls, **when a new log line appears** (the `status | playing asset id | expected asset id` key, and the fact that a schedule rollover prints a fresh `match` line even when nothing is wrong), what each verdict means, and why `unscheduled` is usually an ad break. Written after the operator asked exactly these questions in use — the panel's behaviour was defensible but not self-evident.
- **Print schedule** (added 2026-08-28) dumps the channel's airings on their own — no playback URL, no monitoring run. `formatScheduleListing()` in `epg.js` returns column-aligned plain lines (index, start UTC, duration, asset id, TMS id, title) with a `▶` on the airing covering *now*; `buildScheduleCsv()` is the same listing as CSV. Both are pure and source-agnostic, so XMLTV prints identically to Gracenote. Worth having on its own: it's how you sanity-check that a `prgSvcId` is the channel you think it is *before* a drift run (see the pairing problem below), what you read afterwards to see what was meant to be airing, and the only way to hand the airings to someone with no key of their own. While a run is in progress it prints the already-loaded schedule instead of re-fetching — same channel and window, and a second fetch would spend the user's quota and interleave with the live log.
- **Credentials**: the user's own Gracenote key, `type="password"`, remembered in that browser's `localStorage` behind the "Remember in this browser" checkbox (see the credential-policy section above for the trade). Every `localStorage` access is guarded — it throws outright when storage is disabled (Safari private browsing, blocked cookies), and a browser that won't persist should cost a retype, not a broken panel. Error paths run through `redact()`; e2e tests assert the key never reaches the DOM, that it survives a reload, and that unticking forgets it immediately.
- **Gracenote serialization is verified** (2026-08-28, live response for prgSvcId 157905): `date="2026-08-28"` on `<schedule>`, `time="00:19"` (HH:MM, no seconds) and **`dur="PT00H28M"` — an ISO 8601 duration** on `<event>`. That last one was a real bug: durations parsed to `null`, nulling `stopMs`, so `findScheduledAt` matched nothing and it presented as "no schedule" rather than a parse failure. `tests/fixtures/gracenote-schedule-real.xml` is a trimmed real capture pinning this shape (responses carry no credential — `api_key` isn't echoed back). Gracenote `<error>` bodies still surface verbatim.
...
### 5. Validate a VAST/VMAP ad response (shipped 2026-08-18, Phase 4)
...
## Explainer modals ("How it works")

Separate from the glossary and deliberately so: the glossary defines a *term* in one sentence from a data map, while an explainer describes how a *panel* works in several paragraphs of static markup. `explainer-ui.js` opens a shared `#explainer-modal` by cloning whichever `<template>` a button's `data-explainer` attribute names, with the heading taken from the template's `data-title`. Event-delegated, so any panel can opt in by adding a button and a template to index.html with no JS change. The clone is dropped on close, so find-in-page and screen readers never see two copies of the text.

Only the EPG panel has one so far (`#epg-explainer`). The content is authored markup, never anything fetched or user-supplied — the one place in this app where markup is cloned wholesale rather than escaped, and it stays that way only as long as that remains true.

Note the two modals are independent overlays at the same z-index, so explainer content deliberately does **not** use `.glossary-term` spans — clicking one inside an open explainer would stack the glossary modal on top of it.
...
- **Test suite**: 304 unit + 23 e2e, all passing as of 2026-09-10. See ROADMAP.md Phase 0 and the "Running it" section above. (Counts move constantly — re-run rather than trusting this number.)
