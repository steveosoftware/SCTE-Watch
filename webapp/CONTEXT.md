...
```bash
npm test            # unit tests (fast, no browser, no network) — 283 passing
npm run test:e2e     # e2e — needs `npx playwright install chromium` once first — 22 passing
```

Counts are as of 2026-08-28. The e2e suite's last three tests hit real public streams and can fail on a CDN outage without anything being wrong here — see ROADMAP.md Phase 0.
...
- **The panel does not read PDT even where one exists** — `parseMediaPlaylistAssets()` always interpolates backwards from the live edge, which is why every time it reports is ±1 segment. Corrected 2026-08-28 after the operator pointed out that the earlier wording ("these streams carry no PDT") generalized from Xumo to every stream, and that the drift log line asserted `no PROGRAM-DATE-TIME in this stream` without ever checking. **Open improvement**: prefer a real PDT when present, fall back to the live edge otherwise — that would make drift exact on those streams, and `scte35.js`'s `findCueWallclocks()` already does precisely this for cue timing.
